...
export default function PaymentReceiptModal({ appointment, doctor, onClose }) {
  ...
  const handlePrint = () => {
    ...
    const afip_html = appointment.afip_cae ? `
    <div class="afip-box">
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .afip-box strong { font-weight: 700; }
  ...
  <div class="receipt-content">
    ...
    <div class="grid-2">
      <div>
        ...
        <div class="info-value">${appointment.patient || '—'}</div>
        <div class="info-sub">Fecha Emisión: ${now.toLocaleDateString('es-AR', { day: '2-digit', month: '2-digit', year: 'numeric' })}</div>
      </div>
      <div>
        <div class="info-label">Detalle del Turno</div>
        <div class="info-value">${doctor?.name || '—'}</div>
        <div class="info-sub">${doctor?.specialty ? `Especialidad: ${doctor.specialty}` : '—'}</div>
        <div class="info-sub" style="margin-top:4px;">${appointmentDate} - ${appointment.time || ''}</div>
      </div>
    </div>
    ...
    <table class="items-table">
      ...
      <tbody>
        <tr>
          <td class="service-title">${appointment.title || 'Consulta Médica'}</td>
    ...
    <div class="summary-section">
      ...
      <div class="total-block">
        ...
        <div class="total-amount">${formatMoney(amount)}</div>
      </div>
    </div>
      ...
      <div className="bg-white rounded-[32px] w-full max-w-[460px] shadow-2xl flex flex-col overflow-hidden animate-fade-in-up border border-slate-200/50">
        ...
        <div className="flex items-center justify-between px-8 py-5 border-b border-slate-100 bg-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-slate-900 text-base tracking-tight">Comprobante</h3>
        ...
        <div ref={receiptRef} className="overflow-y-auto max-h-[65vh] p-8 space-y-8 bg-white custom-scrollbar">
          ...
          <div className="grid grid-cols-2 gap-6">
             <div>
                <p className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mb-1.5">Facturado A</p>
                <p className="text-sm font-bold text-slate-800 leading-tight">{appointment.patient || '—'}</p>
                <p className="text-[11px] text-slate-500 mt-1 font-medium">{now.toLocaleDateString('es-AR')}</p>
             </div>
             <div>
                <p className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mb-1.5">Detalle del Turno</p>
                <p className="text-sm font-bold text-slate-800 leading-tight">{doctor?.name || '—'}</p>
                {doctor?.specialty && <p className="text-[11px] text-slate-500 mt-1 font-medium">{doctor.specialty}</p>}
                <p className="text-[11px] text-slate-500 mt-1 font-medium">{appointmentDate} - {appointment.time || ''}</p>
             </div>
          </div>
          ...
          <div className="bg-slate-50/80 rounded-3xl p-6 flex justify-between items-center border border-slate-100">
             <div>
                <p className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mb-1.5">Total Abonado</p>
                <p className="text-3xl font-extrabold text-indigo-600 tracking-tighter">{formatMoney(amount)}</p>
             </div>
          ...
          {appointment.afip_cae && (
            <div className="bg-emerald-50 rounded-2xl p-4 border border-emerald-100 space-y-2">
              <div className="flex items-center gap-2 text-[9px] font-extrabold text-emerald-600 uppercase tracking-widest mb-2">
                <Landmark size={12} /> Información Fiscal AFIP
              </div>
              <div className="flex justify-between items-center text-[11px]">
                <span className="text-emerald-700/70 font-bold">Comprobante</span>
                <span className="font-bold text-emerald-800">
                  {String(appointment.afip_punto_venta || 1).padStart(5, '0')}-{String(appointment.afip_nro).padStart(8, '0')}
                </span>
              </div>
              <div className="flex justify-between items-center text-[11px]">
                <span className="text-emerald-700/70 font-bold">CAE</span>
                <span className="font-bold text-emerald-800">{appointment.afip_cae}</span>
              </div>
              <div className="flex justify-between items-center text-[11px]">
                <span className="text-emerald-700/70 font-bold">Vto. CAE</span>
                <span className="font-bold text-emerald-800">{appointment.afip_cae_vence}</span>
              </div>
            </div>
          )}
        ...
        <div className="p-6 border-t border-slate-100 bg-slate-50/50">
          <button
            onClick={handlePrint}
            className="w-full py-4 text-xs font-extrabold bg-slate-900 text-white hover:bg-indigo-600 rounded-2xl transition-all shadow-lg shadow-slate-900/10 flex items-center justify-center gap-2 uppercase tracking-widest"
          >
